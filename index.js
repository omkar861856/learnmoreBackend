...
app.post("/signup",async function(request,response){
    ...
    let userdb = await client.db('LT').collection("Users").findOne({email:email});
    if(userdb){
        response.status(200).send({msg:"User already present"});
    }else{
        const hashedPass = await hashedPassword(password);
        let result = await client.db('LT').collection("Users").insertOne({email:email, password:hashedPass})
